...
import { getElectionResults, getElectionDetails } from '../services/apiService';
...
function ElectionResults() {
        ...
        const fetchData = async () => {
            ...
            try {
                const [detailsRes, resultsRes] = await Promise.all([
                    getElectionDetails(activeElectionId),
    ...
    const groupedAndSortedResults = useMemo(() => {
        ...
        const grouped = resultsList.reduce((acc, result) => {
            const key = result.positionName || 'Unknown Position'; // Use positionName from DTO
            acc[key] = acc[key] || [];
            acc[key].push(result);
            return acc;
        }, {});
